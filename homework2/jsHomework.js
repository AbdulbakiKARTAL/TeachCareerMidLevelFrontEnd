...
for (let i = 0; i < products.length; i++) {
    30 < products[i].unitPrice && products[i].unitPrice < 100 ? numberOfProduct += 1 : ''
}
...
var startWithAList = []
for (let i = 0; i < products.length; i++) {
    let productsName = products[i].name.split(" ");
    let productName = productsName[0].split("")
    productName[0].toLowerCase() === 'a' ? startWithAList.push(products[i].name) : ''
}
...
for (let i = 0; i < products.length; i++) {
    let productsName = products[i].name.split(" ");
    let productName = productsName[productsName.length - 1].split("");
    productName[productName.length - 1].toLowerCase() === 'a' ? finishWithAList.push(products[i].name) : ''
}

// id si 10 olan ürünün adını console a yazdır
var productName = ''
for (let i = 0; i < products.length; i++) {
    products[i].id === 10 ? productName=products[i].name : ''
}

//discontinued false olan kaç ürün var
let discontinuedFalseNumber = 0
for (let i = 0; i < products.length; i++) {
    products[i].discontinued === false ? discontinuedFalseNumber += 1 : ''
}

// en pahalı ürünün adını console a yazdır
var mostExpensiveProductName = ''
var mostExpensivePrice = products[0].unitPrice

for (let i = 1; i < products.length; i++) {

    if (products[i].unitPrice > mostExpensivePrice) {
        mostExpensivePrice = products[i].unitPrice
        mostExpensiveProductName = products[i].name
    }
}

//en ucuz ürünün adını console a yazdır
var cheapestProductName = ''
var cheapestProductPrice = products[0].unitPrice
for (let i = 1; i < products.length; i++) {
    if (products[i].unitPrice < cheapestProductPrice) {
        cheapestProductName = products[i].name
        cheapestProductPrice = products[i].unitPrice
    }
}

var allAnswersList = {
    'answer1':numberOfProduct,
    'answer2':startWithAList,
    'answer3':finishWithAList,
    'answer4':productName,
    'answer5':discontinuedFalseNumber,
    'answer6':mostExpensiveProductName,
    'answer7':cheapestProductName
}
